  ...
  class View
    module Window
      class Base
        ...
        def normalize_line
          in_pos(cursor.y, cursor.x) { |win| win.draw_at(win.cursor.y + win.top_statement) }
          refresh
        end

        def enhansive_line
          in_color (Tiqbi::F_YELLOW_B_BLACK) { |win|
            in_pos(cursor.y, cursor.x) { |win| win.draw_at(win.cursor.y + win.top_statement, false) }
            refresh
          }
        end
        ...
        def_delegators :@c_window, :clear, :addstr, :maxx, :maxy, :begx, :begy, :box, :border, :subwin, :standout, :standend, :getch, :setpos, :scrl, :resize, :clrtoeol
      end
    end
  end
end
